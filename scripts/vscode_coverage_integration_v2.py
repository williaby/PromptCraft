...
class VSCodeCoverageIntegration:
    ...
    def ensure_xml_reports(self) -> bool:
        ...
        try:
            ...
            need_xml = (
                not self.coverage_xml.exists() or
                self.coverage_xml.stat().st_mtime < self.coverage_file.stat().st_mtime
            )
            ...
            need_html = (
                not self.htmlcov_dir.exists() or
                not (self.htmlcov_dir / "index.html").exists() or
                (self.htmlcov_dir / "index.html").stat().st_mtime < self.coverage_file.stat().st_mtime
            )
    ...
    def run_automation_scripts(self) -> bool:
        ...
        try:
            ...
            ], cwd=self.project_root, capture_output=True, text=True, timeout=45)
