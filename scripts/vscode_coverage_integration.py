    ...
    def _generate_test_type_links(self) -> str:
        ...
        for test_type, display_name in test_types.items():
            ...
            if test_type_index.exists():
                links.append(
                    f'<li><strong><a href="by-type/{test_type}/">{display_name}</a></strong> - Function/class level coverage</li>',
                )
            else:
                # Still show the link but indicate it's pending generation
                links.append(
                    f'<li><span style="color: #6c757d;">{display_name}</span> - Will be available after enhanced report generation</li>',
                )
