...
class ZenMCPClient:
    ...
    def _get_prompt_template(self, assumption: 'Assumption') -> VerificationPrompt:
        ...
        if assumption.risk_level == 'critical' and any(word in category_lower for word in ['security', 'auth', 'authentication', 'authorization']):
            return self.templates.get_critical_security_prompt()
        
        # Critical payment assumptions
        if assumption.risk_level == 'critical' and any(word in category_lower for word in ['payment', 'financial', 'money', 'transaction']):
    ...
    def _select_model(self, template: VerificationPrompt, assumption: 'Assumption') -> str:
        ...
        elif self.budget == 'premium':
            return template.model_requirements.get('preferred', 'gemini-2.5-pro')
        else:  # balanced
            # Use preferred for critical, fallback for others
    ...
    def _make_resilient_mcp_request(self, prompt: str, model: str, assumption: 'Assumption') -> Dict[str, Any]:
        ...
        elif assumption.risk_level == 'critical':
            return {
                'status': 'BLOCKING',
                'confidence': 0.9,
                'issues_found': [
...
def main():
    ...
    print(f"Strategy: tiered, Budget: balanced")
