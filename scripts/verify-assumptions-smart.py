...
class FileCollector:
    ...
    def collect_files(self) -> List[Path]:
        ...
        elif self.scope == "changed-files":
            return self._get_changed_files()
        elif self.scope == "all-files":
    ...
    def _get_changed_files(self) -> List[Path]:
        ...
        try:
            ...
            print(f"⚠️ Warning: Git operation failed (exit code {e.returncode}), falling back to full scan", file=sys.stderr)
            return self._get_all_source_files()
...
class VerificationReporter:
    ...
    def generate_report(self, assumptions: List[Assumption], 
                       verification_results: Optional[List['AIVerificationResult']] = None,
                       args: argparse.Namespace = None) -> str:
    ...
    def _generate_markdown_report(self, assumptions: List[Assumption], 
                                 verification_results: Optional[List['AIVerificationResult']], 
    ...
    def _generate_json_report(self, assumptions: List[Assumption], 
                             verification_results: Optional[List['AIVerificationResult']], 
                             args: argparse.Namespace) -> str:
    ...
    def _generate_text_report(self, assumptions: List[Assumption], 
                             verification_results: Optional[List['AIVerificationResult']], 
                             args: argparse.Namespace) -> str:
    ...
    def _find_ai_result(self, assumption: Assumption, 
                       verification_results: Optional[List['AIVerificationResult']]) -> Optional['AIVerificationResult']:
