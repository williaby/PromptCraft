...
class TestDatetimeMigrationTool:
    ...
    def __init__(self):
        ...
        self.test_replacements = [
            ...
            (
                r'(\w+\._cache_timestamp\s*=\s*)datetime\.now\(\)',
                r'\1utc_now()'
            ),
            # Authentication timestamps 
            (
                r'("authenticated_at":\s*)datetime\.now\(\)',
                r'\1utc_now()'
            ),
            # General variable assignments
            (
                r'(\w+\s*=\s*)datetime\.now\(\)',
                r'\1utc_now()'
            ),
            # Direct datetime.now() calls
            (
                r'datetime\.now\(\)(?!\s*-|\s*\+)',  # Not followed by arithmetic
                r'utc_now()'
            ),
            # Arithmetic with datetime.now() - keep for MockDatetime context
            (
                r'datetime\.now\(\)(\s*[-+]\s*timedelta\([^)]+\))',
                r'utc_now()\1'
            )
        ]
