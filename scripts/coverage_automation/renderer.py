...
class CoverageRenderer:
    ...
    def generate_coverage_reports(self, used_contexts: Set[str]) -> str:
        ...
        try:
            ...
            if gap_analysis_path:
                self.logger.info(
                    "Test gap analysis generated",
                    gap_analysis_path=str(gap_analysis_path)
                )
            ...
            self.logger.error(
                "Error generating coverage reports",
                error=str(e)
            )
            return ""
    ...
    def _combine_coverage_files(self) -> None:
        ...
        try:
            ...
            ], cwd=self.project_root, check=False, capture_output=True, text=True)
            ...
            self.logger.warning(
                "Error combining coverage files",
                error=str(e)
            )
...
"""
        ...
        if context.test_types:
            for test_type in sorted(context.test_types):
                if test_type in self.test_types:
                    ...
                    type_data = context.coverage_by_type.get(test_type, {'statement': 0, 'branch': 0, 'total_branches': 0})
    ...
    def _generate_test_type_html(self, test_type: str, info: Dict, filtered_files: Dict, totals: Dict) -> str:
        ...
        coverage_pct = ((totals['statements'] - totals['missing']) / totals['statements'] * 100) if totals['statements'] > 0 else 0
...
<body>
        ...
        for asset in assets:
            ...
            if src.exists() and not dst.exists():
                try:
                    ...
                    self.logger.warning(
                        "Could not copy coverage asset",
                        asset=asset,
                        error=str(e)
                    )
    
    def _generate_test_gap_analysis(self, coverage_data: Dict[str, Any]) -> Optional[Path]:
        ...
        try:
            ...
            self.logger.error(
                "Error generating test gap analysis",
                error=str(e)
            )
            return None
    ...
    def _generate_gap_analysis_html(self, coverage_data: Dict[str, Any]) -> str:
        ...
        low_coverage_files = sum(
            ...
            if file_data.get('summary', {}).get('percent_covered', 0) < 50
        )
