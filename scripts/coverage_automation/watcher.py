...
class CoverageWatcher:
    ...
    def _find_coverage_file(self) -> Path:
        ...
        if not coverage_file.exists():
            ...
            if coverage_files:
                ...
                self.logger.info("Using coverage file with hostname suffix", coverage_file=str(coverage_file))
    ...
    def detect_vscode_coverage_run(self, force_run: bool = False) -> bool:
        ...
        try:
            ...
            if not self.coverage_file.exists():
                self.logger.warning("Coverage file not found", coverage_file=str(self.coverage_file))
                return False
            ...
            self.logger.error("Error detecting coverage run", error=str(e), coverage_file=str(self.coverage_file))
            return False

        finally:
            self.perf_logger.log_operation_timing("detect_vscode_coverage_run", time.time() - start_time)
    ...
    def get_coverage_contexts(self) -> set[str]:
        ...
        try:
            ...
            self.logger.error("Error detecting coverage contexts", error=str(e))
        ...
        try:
            if self.coverage_json_path.exists():
                ...
                if total_coverage > 80 and len(files) > 30:
                    ...
                    file_patterns = {"auth": "auth/", "security": "security/", "examples": "examples/"}
            ...
            self.logger.debug("Could not analyze coverage data", error=str(e))
            ...
            for cache_file in pytest_cache.rglob("*"):
                if cache_file.is_file():
                    try:
                        ...
                        if file_age < 3600:  # 1 hour
                            self.logger.debug("Recent pytest cache detected", file_age_seconds=round(file_age, 2))
