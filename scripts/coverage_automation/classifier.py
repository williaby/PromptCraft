...
from typing import Dict, Set, FrozenSet, List, Any
from .types import TestFileAnalysis, TestTargetMapping
from .config import TestPatternConfig
from .security import SecurityValidator
from .logging_utils import get_logger, get_security_logger, get_performance_logger
...
class TestTypeClassifier:
        ...
        try:
            ...
            for test_type in contexts:
                if test_type not in file_patterns:
                    self.logger.warning(
                        "Test type not found in patterns",
                        test_type=test_type
                    )
                    continue
                ...
                if matching_files:
                    ...
                    self.logger.debug(
                        "No matching files found for test type",
                        test_type=test_type
                    )
            ...
            self.logger.error(
                "Error estimating test type coverage",
                error=str(e),
                contexts=list(contexts)
            )
            # Return empty coverage for all contexts to avoid breaking the system
            return {test_type: {'statement': 0.0, 'branch': 0.0, 'total_branches': 0} 
                    for test_type in contexts}
    ...
    def get_test_target_mapping(self, test_type: str) -> FrozenSet[str]:
        ...
        try:
            ...
            self.logger.error(
                "Error getting test target mapping",
                test_type=test_type,
                error=str(e)
            )
            return frozenset()
