...
class SecurityValidator:
    ...
    def validate_path(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if not resolved_path.is_relative_to(self.project_root):
                self.security_logger.log_path_validation_failure(file_path, "Path outside project bounds")
                return False
            ...
            self.security_logger.log_path_validation_failure(file_path, f"Path resolution error: {e}")
            return False
    ...
    def validate_file_size(self, file_path: Path, max_size_mb: float = 1.0) -> bool:
        ...
        try:
            ...
            if file_size > max_size_bytes:
                self.security_logger.log_file_size_violation(file_path, file_size, max_size_bytes)
                return False

            return True

        except (OSError, FileNotFoundError) as e:
            self.security_logger.log_path_validation_failure(file_path, f"File access error: {e}")
            return False
    ...
    def sanitize_content(self, content: str) -> str:
        ...
        sanitized = "".join(char for char in content if char in string.printable)
    ...
    def validate_import_path(self, import_path: str) -> bool:
        ...
        if ".." in import_path:
            self.security_logger.log_import_path_rejection(import_path, "Path traversal attempt")
            return False

        # Check for directory separators in import paths (only actual slashes, not dots)
        if "/" in import_path or "\\" in import_path:
            self.security_logger.log_import_path_rejection(import_path, "Invalid directory separators")
            return False

        # Limit reasonable module path length
        if len(import_path) > 100:
            self.security_logger.log_import_path_rejection(import_path, "Import path too long")
            return False

        # Only allow valid module names
        if not self.valid_module_pattern.match(import_path):
            self.security_logger.log_import_path_rejection(import_path, "Invalid module name format")
            return False
