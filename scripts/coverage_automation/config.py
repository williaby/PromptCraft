    ...
    def _load_codecov_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if codecov_path.exists():
                with open(codecov_path, 'r', encoding='utf-8') as f:
                    codecov_config = yaml.safe_load(f)
                
                self.logger.info(
                    "Codecov configuration loaded for flag alignment",
                    codecov_path=str(codecov_path)
                )
            ...
            self.logger.error(
                "Error loading codecov configuration",
                error=str(e)
            )
            return {}
