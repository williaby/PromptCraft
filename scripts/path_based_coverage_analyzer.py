...
class IntelligentTestTypeClassifier:
    ...
    def get_display_info(self, test_type: str) -> Dict[str, str]:
        ...
        return {
            'display_name': f'❓ {test_type.title()} Tests',
            'icon': '❓'
        }
    ...
    def parse_index_html(self) -> List[CoverageFileData]:
        ...
        for row in tbody.find_all('tr', class_='region'):
            try:
                ...
                coverage_files.append(CoverageFileData(
                    file_path=file_path,
...
class PathBasedCoverageAnalyzer:
    ...
    def analyze_coverage_by_test_types(self) -> Dict[str, TestTypeAnalysis]:
        ...
        for test_type, analysis in test_analyses.items():
            ...
            if file_count > 0:
                self.log(f"  {analysis.display_name}: {file_count} files, "
                        f"{analysis.coverage_percent:.1f}% coverage")
    ...
    def _generate_individual_report(self, test_type: str, analysis: TestTypeAnalysis, 
                                   by_type_dir: Path) -> None:
    ...
    def _generate_test_type_html(self, analysis: TestTypeAnalysis, 
                                sorted_files: List[CoverageFileData]) -> str:
...
<body>
    <div class="container">
        ...
        for file_data in sorted_files:
            ...
            if file_data.detailed_link:
                file_link = f'<a href="../../standard/{file_data.detailed_link}" class="file-link">{file_data.file_path}</a>'
            else:
    ...
    def _generate_overview_index(self, test_analyses: Dict[str, TestTypeAnalysis], 
                                by_type_dir: Path) -> None:
...
<body>
    ...
    def _create_main_dashboard(self, test_analyses: Dict[str, TestTypeAnalysis], 
        ...
        if total_files > 0:
            weighted_coverage = sum(
                analysis.coverage_percent * len(analysis.files) 
...
<body>
    ...
    def run_analysis(self) -> bool:
        ...
        try:
            ...
            for test_type, analysis in test_analyses.items():
                if analysis.files:
                    print(f"  {analysis.display_name}: {analysis.coverage_percent:.1f}% "
