...
class RiskScore:
    ...
    def __init__(self, score: int = 0, factors: list[str] | None = None, confidence_score: float = 0.0,
                 user_id: str | None = None, overall_score: float | None = None, 
                 risk_factors: dict | None = None, timestamp=None) -> None:
...
class BehaviorProfile:
    ...
    def __init__(self, 
        ...
        self.last_updated: datetime = last_updated or datetime.now(timezone.utc)
...
class SuspiciousActivityDetector:
    ...
    async def _get_user_pattern(self, user_id: str | None) -> UserPattern | None:
        ...
        if user_id not in self.user_patterns:
            self.user_patterns[user_id] = UserPattern(user_id=user_id, first_seen=datetime.now(timezone.utc))
    ...
    async def _update_user_pattern(self, event: SecurityEventCreate, user_pattern: UserPattern | None) -> None:
        ...
        user_pattern.last_updated = datetime.now(timezone.utc)
    ...
    async def build_behavior_profile(self, user_id: str) -> BehaviorProfile:
        ...
        if not user_pattern:
            user_pattern = UserPattern(user_id=user_id, first_seen=datetime.now(timezone.utc))
    ...
    async def analyze_time_pattern(self, user_id: str, timestamp: datetime) -> float:
        ...
        if total_hour_logins > 0:
            ...
            elif effective_frequency < 0.05:  # Less than 5% of typical logins
                return 0.7  # High suspicion
            elif effective_frequency > 0.08:  # More than 8% of typical logins - normal pattern
    ...
    async def analyze_velocity_pattern(self, user_id: str, recent_events: list[Any]) -> float:
        ...
        for i in range(1, len(sorted_events)):
            ...
            logger.debug(f'Event1 has metadata: {hasattr(event1, "metadata")}, has details: {hasattr(event1, "details")}')
            logger.debug(f'Event1 metadata: {getattr(event1, "metadata", None)}, details: {getattr(event1, "details", None)}')
            logger.debug(f'Event2 has metadata: {hasattr(event2, "metadata")}, has details: {hasattr(event2, "details")}')
            logger.debug(f'Event2 metadata: {getattr(event2, "metadata", None)}, details: {getattr(event2, "details", None)}')
            ...
            if (loc1.get('lat') and loc1.get('lon') and 
                loc2.get('lat') and loc2.get('lon')):
    ...
    async def detect_frequency_anomaly(self, user_id: str, events: list[Any]) -> float:
        ...
        elif len(short_intervals) > len(intervals) * 0.5:  # More than 50% burst
            return 0.7  # Medium frequency anomaly
        elif len(short_intervals) > len(intervals) * 0.3:  # More than 30% burst
    ...
    async def detect_pattern_anomaly(self, user_id: str, events: list[Any]) -> float:
        ...
        elif hour_range > 15 or unique_hours > 8:  # Moderately spread
            return 0.6  # Medium pattern deviation
        elif hour_range > 10 or unique_hours > 5:  # Somewhat spread
    ...
    async def classify_risk_level(self, risk_score: float) -> str:
        ...
        if risk_score > 0.80:       # Critical risk threshold (>0.8)
            return "CRITICAL"
        elif risk_score > 0.50:     # High risk threshold (>0.5)  
            return "HIGH"
        elif risk_score > 0.20:     # Medium risk threshold (>0.2)
            return "MEDIUM"
        else:
            return "LOW"
    ...
    async def calculate_risk_trend(self, user_id: str, time_window_days: int = 7) -> dict[str, Any]:
        ...
        if len(historical_scores) >= 2:
            ...
            sum_xy = sum(x * y for x, y in zip(time_points, scores_array))
    ...
    async def analyze_activity_batch(self, user_id: str, events_batch: list[SecurityEvent]) -> list[ActivityAnalysisResult]:
    ...
    async def analyze_streaming_window(self, user_id: str, events: list[SecurityEvent], window_size_minutes: int = 60) -> dict[str, Any]:
        ...
        return {
            ...
            "average_anomaly_score": sum(score["anomaly_score"] for score in anomaly_scores) / len(anomaly_scores) if anomaly_scores else 0.0
        }
    ...
    async def _calculate_event_anomaly_score(self, event: SecurityEvent, window_events: list[SecurityEvent]) -> float:
        ...
        unique_ips = len(set(e.ip_address for e in window_events if e.ip_address))
    ...
    async def detect_ml_anomaly(self, user_id: str, features: list[float] | dict) -> float:
        ...
        normal_features = [f for f, label in zip(training_data['features'], training_data['labels']) if not label]
        ...
        for normal_example in normal_features:
            if len(feature_values) == len(normal_example):
                distance = sum((f1 - f2) ** 2 for f1, f2 in zip(feature_values, normal_example)) ** 0.5
                min_distance = min(min_distance, distance)
        ...
        for key, risk_score in self._risk_scores.items():
            if key.startswith(f"{user_id}_") and risk_score.timestamp:
                ...
                if risk_timestamp.tzinfo is None:
                    risk_timestamp = risk_timestamp.replace(tzinfo=timezone.utc)
    ...
    async def calculate_contextual_risk_score(
        ...
        if "privileged_access" in context and context["privileged_access"]:
    ...
    async def analyze_behavioral_patterns(
        ...
        logger.info(f"Analyzed behavioral patterns: {len(patterns)} patterns found with min_confidence={min_confidence}")
