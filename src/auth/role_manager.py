            ...
            async with self.get_session() as session:
                ...
                if parent_role_name:
                    parent_result = await session.execute(
                        select(Role.id).where(Role.name == parent_role_name, Role.is_active.is_(True)),
                    )
        ...
        try:
            async with self.get_session() as session:
                result = await session.execute(
                    select(Role).where(Role.name == role_name, Role.is_active.is_(True)),
                )
        ...
        try:
            async with self.get_session() as session:
                ...
                role_result = await session.execute(
                    select(Role.id).where(Role.name == role_name, Role.is_active.is_(True)),
                )
            ...
            async with self.get_session() as session:
                ...
                if not force:
                    ...
                    child_result = await session.execute(
                        select(Role.name).where(Role.parent_role_id == role.id, Role.is_active.is_(True)),
                    )
