...
class AgentResourceManager:
    ...
    def can_load_agent(self, agent_def: AgentDefinition) -> bool:
        ...
        if len(self.active_agents) >= self.resource_limits["max_concurrent"]:
            return False
        ...
        if current_usage + required > self.resource_limits["total_memory"]:
            return False

        # Check model limits
        model = agent_def.runtime.model
        active_count = sum(1 for agent in self.active_agents.values() if agent["model"] == model)
        return not active_count >= self.resource_limits["model_limits"].get(model, 1)
    ...
    def get_resource_usage(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memory_utilization": total_memory / self.resource_limits["total_memory"],
...
class DynamicAgentLoader(LoggerMixin):
    ...
    def load_markdown_agent(self, agent_def: AgentDefinition, config: dict[str, Any]) -> BaseAgent:
        ...
        if agent_def.implementation.source is None:
            raise ValueError(f"Markdown agent {agent_def.id} has no source definition")

        return MarkdownAgent(
            agent_id=agent_def.id,
            definition=agent_def.implementation.source,
            model=agent_def.runtime.model,
    ...
    def load_python_agent(self, agent_def: AgentDefinition, config: dict[str, Any]) -> BaseAgent:
        ...
        try:
            ...
            instance: BaseAgent = agent_class(config)
            return instance
        except Exception as e:
