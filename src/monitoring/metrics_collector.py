...
class MetricsDatabase:
    ...
    def _initialize_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_aggregated_window ON aggregated_metrics(window_start, window_end)",
            )
            conn.execute("CREATE INDEX IF NOT EXISTS idx_validation_created ON validation_results(created_at)")

    async def store_metric_point(self, point: MetricPoint) -> None:
...
class MetricsAggregator:
    ...
    def _group_points_by_windows(
        self,
        points: list[MetricPoint],
        window_duration: timedelta,
        start_time: datetime,
        end_time: datetime,
    ) -> list[tuple[datetime, datetime, list[MetricPoint]]]:
        ...
        while current_window_start < end_time:
            ...
            window_points = [point for point in points if current_window_start <= point.timestamp < current_window_end]
    ...
    def _calculate_aggregation(  # noqa: PLR0911  # Aggregation type mapping requires multiple returns
        ...
        if agg_type == MetricAggregationType.SUM:
            return sum(values)
        if agg_type == MetricAggregationType.AVERAGE:
            return statistics.mean(values)
        if agg_type == MetricAggregationType.MEDIAN:
            return statistics.median(values)
        if agg_type == MetricAggregationType.COUNT:
            return float(len(values))
        if agg_type == MetricAggregationType.MIN:
            return min(values)
        if agg_type == MetricAggregationType.MAX:
            return max(values)
        if agg_type == MetricAggregationType.PERCENTILE_95:
            return float(np.percentile(values, 95))
        if agg_type == MetricAggregationType.PERCENTILE_99:
            return float(np.percentile(values, 99))
        if agg_type == MetricAggregationType.STANDARD_DEVIATION:
...
class StatisticalValidator:
    ...
    async def validate_token_reduction_claim(
        self,
        ...
        points = await self.database.get_metric_points("token_reduction_percentage", start_time, end_time)
    ...
    async def validate_performance_claims(self) -> list[ValidationResult]:
        ...
        for claim_name, metric_name, target_value, comparison in claims_to_validate:
            result = await self._validate_single_claim(claim_name, metric_name, target_value, comparison)
...
class MetricsCollector:
    ...
    async def generate_comprehensive_report(self, time_period: timedelta = timedelta(days=7)) -> dict[str, Any]:
        ...
        return {
            ...
            "statistical_summary": {
                "sample_sizes": {
                    metric: len(await self.database.get_metric_points(metric, current_time - time_period, current_time))
                    for metric in ["token_reduction_percentage", "loading_latency_ms", "success_rate_percentage"]
                },
