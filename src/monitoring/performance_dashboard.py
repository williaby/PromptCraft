...
class RealTimeDashboard:
    ...
    async def _generate_dashboard_data(self) -> dict[str, Any]:
        ...
        validation_details = {
            ...
            "criteria_status": {
                ...
                "task_accuracy_acceptable": self._get_health_value(health_report, "task_detection_accuracy_rate")
                >= 0.80,
    ...
    async def _generate_alerts(self, health_report: Any) -> list[dict[str, Any]]:
        ...
        if (
            ...
            < self.monitor.min_acceptable_reduction * 100
        ):
            alerts.append(
                {
                    "level": "warning",
...
class AlertManager:
    ...
    async def check_alerts(self) -> list[dict[str, Any]]:
        ...
        if (
            self._get_health_value(health_report, "average_token_reduction_percentage")
            < self.thresholds["token_reduction_min"]
        ):
            alert = {
            ...
            self._get_health_value(health_report, "average_token_reduction_percentage")
            < self.thresholds["token_reduction_target"]
        ):
            alert = {
    ...
    async def _update_active_alerts(self, current_alerts: list[dict[str, Any]]) -> None:
...
def create_dashboard_app(monitor: TokenOptimizationMonitor) -> FastAPI:
    ...
    @app.get("/api/health")
    async def get_health_report() -> Any:
        return await monitor.generate_system_health_report()

    @app.get("/api/optimization-report")
    async def get_optimization_report(user_id: str | None = None) -> Any:
