    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.config.integrations_enabled:
                self.integration_manager = await initialize_integrations(self.config.integrations_config_path)
        ...
        while self.running:
            try:
                ...
                if isinstance(components_dict, dict):
                    unhealthy_components = [name for name, healthy in components_dict.items() if not healthy]
                else:
...
async def initialize_monitoring_system(
    ...
    monitor = _monitoring_system.monitor
    metrics_collector = _monitoring_system.metrics_collector
    dashboard = _monitoring_system.dashboard

    if monitor is None or metrics_collector is None or dashboard is None:
        raise RuntimeError("Failed to initialize monitoring components")

    return (monitor, metrics_collector, dashboard)
