...
from .mcp_client import MCPClientInterface, MCPConnectionState, MCPHealthStatus, MCPError
...
class ZenStdioMCPClient(MCPClientInterface):
    ...
    def __init__(
        ...
        self.zen_client: Optional[ZenMCPStdioClient] = None
    ...
    async def get_model_recommendations(self, prompt: str) -> Any:
        ...
        try:
            ...
            if result and result.success:
                ...
                from .models import RoutingAnalysis, ModelRecommendation
                
                # Extract primary recommendation
                primary_rec = result.recommendations.primary
                primary_recommendation = ModelRecommendation(
                    model_id=primary_rec.model_id,
                    model_name=primary_rec.model_name,
                    tier=primary_rec.tier,
                    reasoning=primary_rec.reasoning,
                    confidence_score=0.9,  # Zen provides high-confidence recommendations
                    estimated_cost=result.recommendations.cost_comparison.recommended_cost
                )
                ...
                return RoutingAnalysis(
                    ...
                    complexity_level=result.analysis.complexity_level,
    ...
    async def execute_with_routing(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result: ExecutionResult = await self.zen_client.execute_smart(request)
                ...
                return {
                    ...
                    "result": {
                        ...
                        "routing_metadata": {
                            ...
                            "complexity_level": result.result.complexity_level,
                            "selection_reasoning": result.result.selection_reasoning,
                            "cost_optimized": result.result.cost_optimized,
