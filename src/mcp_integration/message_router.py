...
class MCPMessageRouter(LoggerMixin):
    ...
    async def connect_server(self, server_name: str, connection: ActiveConnection) -> bool:
        ...
        try:
            if connection.connection.type == "npx" and connection.process:
                ...
                self.server_streams[server_name] = (reader, writer)
                
                # Initialize the server
                success = await self._initialize_server(server_name, reader, writer)
                if success:
    ...
    async def _initialize_server(self, server_name: str, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> bool:
        ...
        try:
            ...
            if response:
                ...
                server_info_obj = MCPServerInfo(
                    name=server_name,
                    connection=None,  # We'll update this later if needed
    ...
    async def _send_request_and_wait(self, writer: asyncio.StreamWriter, reader: asyncio.StreamReader, request: MCPRequest, timeout: float = 10.0) -> Any | None:
    ...
    async def _query_server_capabilities(self, server_name: str, writer: asyncio.StreamWriter, reader: asyncio.StreamReader) -> None:
    ...
    async def _handle_server_messages(self, server_name: str, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
