...
class Context7Client(LoggerMixin):
    ...
    async def search_documents(self, query: str, limit: int | None = None, filters: dict[str, Any] | None = None) -> Context7SearchResult:
        ...
        try:
            ...
            self.logger.info(f"Context7 search for '{query}': {len(documents)} results in {search_result.search_time:.2f}s")
...
class Context7Integration(LoggerMixin):
    ...
    async def enhanced_document_search(self, query: str, use_context7: bool = True, limit: int = 10) -> dict[str, Any]:
        ...
        self.logger.info(f"Enhanced search for '{query}': {results['total_results']} results from {len(results['sources'])} sources")
        return results
