        ...
        try:
            ...
            if self.process and self.process.stderr and self.process.stderr.readable():
                ...
                try:
                    ...
                    logger.debug(f"Select operation failed (likely Windows): {e}")
...
class ProcessPool:
    ...
    def __init__(self, config: MCPConnectionConfig, pool_size: int = 1) -> None:
        self.config = config
        self.pool_size = pool_size
        self.processes: dict[str, ZenMCPProcess] = {}
        self.current_process_id = "main"

    async def get_process(self, process_id: str | None = None) -> ZenMCPProcess | None:
        """
    ...
    def get_pool_status(self) -> dict[str, MCPConnectionStatus]:
        ...
        return {process_id: process.get_status() for process_id, process in self.processes.items()}
