...
class ComprehensiveEvaluation:
    ...
    def needs_revision(self) -> bool:
        ...
        return (
            ...
            or DiagnosticFlag.SAFETY_CONCERN in self.all_flags
        )
...
class ANCHORQREvaluator(LoggerMixin):
    ...
    def evaluate_prompt(
        ...
        """
        try:
            self.logger.info("Starting ANCHOR-QR-8 evaluation with %s rigor", rigor_level.value)
            ...
            self.logger.error("ANCHOR-QR-8 evaluation failed: %s", e)
    ...
    def _evaluate_confidence_sourcing_accuracy(
        ...
        expert_judgment_count = sum(1 for keyword in EXPERT_JUDGMENT_KEYWORDS if keyword in prompt.lower())
    ...
    def _calculate_create_compliance(
        ...
        verification_issues = sum(
            1 for result in step_results.values() if DiagnosticFlag.VERIFICATION_ISSUE in result.flags
        )
    ...
    def _identify_critical_issues(self, evaluation: ComprehensiveEvaluation) -> list[str]:
        ...
        verification_count = sum(
            1 for result in evaluation.step_results.values() if DiagnosticFlag.VERIFICATION_ISSUE in result.flags
        )
        ...
        low_scoring_steps = [step.value for step, result in evaluation.step_results.items() if result.score < 40.0]
