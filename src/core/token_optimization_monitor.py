...
from src.utils.performance_monitor import PerformanceMonitor, MetricData, MetricType
from src.utils.observability import StructuredLogger, create_structured_logger
...
class TokenOptimizationMonitor:
    ...
    async def start_session_monitoring(self, session_id: str, user_id: str, 
                                     task_type: Optional[str] = None,
                                     optimization_level: OptimizationStatus = OptimizationStatus.CONSERVATIVE) -> None:
    ...
    async def record_function_loading(self, session_id: str, tier: FunctionTier,
                                    functions_loaded: List[str], loading_time_ms: float,
                                    tokens_consumed: int, cache_hit: bool = False) -> None:
        ...
        self.performance_monitor.record_metric(MetricData(
        ...
        if loading_time_ms > self.max_acceptable_latency_ms:
            self.logger.warning(
                f"Function loading exceeded latency threshold",
                session_id=session_id,
                tier=tier.value,
    ...
    async def record_function_usage(self, session_id: str, function_name: str,
                                  success: bool = True, tier: Optional[FunctionTier] = None) -> None:
        ...
        if tier and tier in self.function_metrics:
            ...
            if tier_metrics.functions_loaded > 0:
                tier_metrics.usage_frequency = len(session_metrics.functions_actually_used) / tier_metrics.functions_loaded
    ...
    async def record_task_detection(self, session_id: str, detected_task: str,
                                  confidence: float, actual_task: Optional[str] = None) -> None:
    ...
    async def record_fallback_activation(self, session_id: str, reason: str,
    ...
    async def record_user_override(self, session_id: str, override_type: str,
                                 original_optimization: str, new_optimization: str) -> None:
    ...
    async def end_session_monitoring(self, session_id: str) -> Optional[TokenUsageMetrics]:
        ...
        if session_metrics.baseline_tokens_loaded > 0:
            ...
            if token_reduction >= self.token_reduction_target:
                self.logger.info(
                    f"Token reduction target achieved",
                    session_id=session_id,
                ...
                self.logger.warning(
                    f"Token reduction below minimum threshold",
                    session_id=session_id,
                    reduction_percentage=token_reduction * 100,
        ...
        self.logger.info(
            f"Session monitoring completed",
            session_id=session_id,
    ...
    async def _measure_baseline_tokens(self, task_type: Optional[str] = None) -> int:
    ...
    async def generate_system_health_report(self) -> SystemHealthMetrics:
        ...
        recent_sessions = [
            metrics for metrics in self.session_metrics.values()
    ...
    async def _update_optimization_validation(self, health_metrics: SystemHealthMetrics) -> None:
        ...
        self.logger.info(
            f"Optimization validation update",
    ...
    async def export_metrics(self, format: str = "json", 
        ...
        export_data = {
            ...
            "function_tier_metrics": {
                tier.value: asdict(metrics) for tier, metrics in self.function_metrics.items()
            },
    ...
    async def get_optimization_report(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        for session in relevant_sessions:
            if session.baseline_tokens_loaded > 0:
                savings = (session.baseline_tokens_loaded - session.optimized_tokens_loaded) / session.baseline_tokens_loaded
        ...
        report = {
            ...
            "token_optimization": {
                ...
                "target_achieved": (mean(token_savings) if token_savings else 0.0) >= (self.token_reduction_target * 100)
            },
            "performance_metrics": {
...
_global_monitor: Optional[TokenOptimizationMonitor] = None
