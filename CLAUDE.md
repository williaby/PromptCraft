...
- Python 3.11+ with uv (pinned at 0.9.26)
...
uv run --frozen python src/utils/encryption.py   # Validate env + keys
```
...
- Environment validation runs via `uv run --frozen python src/utils/encryption.py`.
