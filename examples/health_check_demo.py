...
def demonstrate_configuration_status() -> None:
    ...
    console.print(Panel("[bold blue]Phase 5: Configuration Status Model Demo[/bold blue]"))
    
    try:
        # Load current settings
        settings = get_settings(validate_on_startup=False)
        console.print(f"✅ Settings loaded for environment: [bold]{settings.environment}[/bold]")
        ...
        table.add_row("Environment", status.environment, "Current deployment environment")
        ...
        table.add_row("Config Loaded", str(status.config_loaded), "Configuration loaded successfully")
        table.add_row("Encryption Available", str(status.encryption_enabled), "Encryption system status")
        table.add_row("Config Source", status.config_source, "Primary configuration source")
        table.add_row("Validation Status", status.validation_status, "Configuration validation result")
        table.add_row("Secrets Configured", str(status.secrets_configured), "Number of configured secrets")
        table.add_row("API Host", status.api_host, "API server host address")
        table.add_row("API Port", str(status.api_port), "API server port")
        table.add_row("Overall Healthy", str(status.config_healthy), "Computed health status")
        ...
        console.print(f"❌ [bold red]Error generating configuration status:[/bold red] {e}")
        return None
    ...
    try:
        ...
        for key, value in summary.items():
            if key == "healthy":
                ...
                console.print(f"  {key.title().replace('_', ' ')}: [cyan]{value}[/cyan]")
...
def demonstrate_json_serialization(status: ConfigurationStatusModel) -> None:
    ...
    try:
        ...
        sensitive_keywords = [
            ...
            "SecretStr", "super_secret", "api_key_value"
        ]
...
async def demonstrate_http_endpoints() -> None:
    ...
    console.print("[dim]Note: Start the server with 'poetry run python src/main.py' to test live endpoints[/dim]\n")
    ...
    console.print(f"[bold]GET /health:[/bold]")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                console.print("✅ [bold green]Live server detected! Testing real endpoints...[/bold green]")
                
                # Test health endpoint
                health_response = await client.get(f"{base_url}/health")
                console.print(f"[bold]Live /health response (status {health_response.status_code}):[/bold]")
                console.print(f"[dim]{json.dumps(health_response.json(), indent=2)}[/dim]\n")
                
                # Test config health endpoint
                config_response = await client.get(f"{base_url}/health/config")
                console.print(f"[bold]Live /health/config response (status {config_response.status_code}):[/bold]")
                console.print(f"[dim]{json.dumps(config_response.json(), indent=2)}[/dim]")
                
    except (httpx.ConnectError, httpx.TimeoutException):
        console.print("ℹ️  [dim]No live server detected. Start with 'poetry run python src/main.py' to test live endpoints.[/dim]")
...
def demonstrate_security_features() -> None:
    ...
    try:
        ...
        if leaked_secrets:
            console.print(f"❌ [bold red]SECURITY BREACH: Found secrets in output:[/bold red] {leaked_secrets}")
        else:
            console.print("✅ [bold green]SECURE: No secret values found in health check output[/bold green]")
            
        # Check for SecretStr representations
        if "SecretStr" in json_output:
            console.print("❌ [bold red]WARNING: SecretStr representations found[/bold red]")
        else:
            console.print("✅ [bold green]SECURE: No SecretStr representations exposed[/bold green]")
...
def main() -> None:
    ...
    console.print("[bold green]PromptCraft-Hybrid: Health Check Integration Demo[/bold green]")
