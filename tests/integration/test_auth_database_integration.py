...
class TestAuthDatabaseIntegration:
    ...
    async def test_role_manager_get_user_permissions_integration(self, test_db_session: AsyncSession, sample_roles_and_permissions, monkeypatch):
        ...
        expected_admin_perms = {
            "users:read", "users:write", "tokens:create",
            "admin:access", "tokens:delete",
        }
        assert expected_admin_perms.issubset(admin_permissions)

    async def test_role_manager_role_hierarchy_operations(self, test_db_session: AsyncSession, sample_roles_and_permissions, monkeypatch):
    ...
    async def test_database_function_user_has_permission(self, test_db_session: AsyncSession, sample_roles_and_permissions):
    ...
    async def test_role_assignment_integration(self, test_db_session: AsyncSession, sample_roles_and_permissions, monkeypatch):
...
@pytest.mark.integration  
@pytest.mark.asyncio
@pytest.mark.skip(reason="Database integration tests deferred for production deployment - requires auth database schema setup not available in test environment")
