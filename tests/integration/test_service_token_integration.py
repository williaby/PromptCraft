...
async def db_session():
    ...
    async def mock_execute(query, params=None):
        ...
        # Handle COUNT queries for token name existence check and emergency revocation
        elif ("COUNT(*)" in query_str and "service_tokens" in query_str) or ("COUNT(*)" in query_str and "SERVICE_TOKENS" in query_str):
            ...
            elif params and "token_name" in params:
        ...
        elif "ORDER BY USAGE_COUNT" in query_str or "ORDER BY usage_count" in query_str:
        ...
        elif ("SELECT ID, TOKEN_NAME" in query_str and "EXPIRES_AT < NOW()" in query_str):
        ...
        # Handle SELECT queries for service token lookup  
        elif ("SELECT" in query_str and "service_tokens" in query_str) or ("SELECT" in query_str and "SERVICE_TOKENS" in query_str):
            ...
            if not token_name:
                ...
                for obj in reversed(added_objects):
                    ...
                    if "cicd-github-actions" in obj_str:
                        ...
                        break
                    elif "middleware-test-token" in obj_str:
                        token_name = "middleware-test-token" 
                        break
                    elif "integration-test-token" in obj_str:
                        token_name = "integration-test-token"
                        break
                    elif "duplicate-test-token" in obj_str:
            ...
            mock_record.created_at = datetime.now(timezone.utc) - timedelta(days=10)
            mock_record.last_used = datetime.now(timezone.utc) - timedelta(hours=1)
            ...
            if is_expiration_query:
                ...
                def create_fresh_expiring_records():
                    ...
                    fresh_record.expires_at = datetime.now(timezone.utc) + timedelta(days=3)
                    fresh_record.usage_count = 10
                    fresh_record.last_used = datetime.now(timezone.utc) - timedelta(hours=1)
                ...
                print(f"MOCK: Set fetchall to dynamically create fresh records")
                ...
                mock_record.created_at = datetime.now(timezone.utc) - timedelta(days=100)
                mock_record.usage_count = 500
                mock_record.last_used = datetime.now(timezone.utc) - timedelta(days=5)
                ...
                mock_record.expires_at = datetime.now(timezone.utc) + timedelta(days=3)
        ...
        # Handle authentication events queries  
            ...
            if "COUNT(*)" in query_str:
                ...
                mock_emergency_event.created_at = datetime.now(timezone.utc)
