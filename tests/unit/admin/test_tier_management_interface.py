...
def mock_tier_manager():
    ...
    manager.get_all_users.return_value = {
        "admin": [{"email": "admin@example.com", "can_access_premium": True, "is_admin": True, "assigned_at": "existing"}],
        "full": [{"email": "full@example.com", "can_access_premium": True, "is_admin": False, "assigned_at": "existing"}],
        "limited": [{"email": "limited@example.com", "can_access_premium": False, "is_admin": False, "assigned_at": "existing"}],
        "unassigned": [{"email": "unassigned@example.com", "can_access_premium": False, "is_admin": False, "assigned_at": "unassigned"}]
    }
    ...
    manager.bulk_assign_tier.return_value = {"total": 2, "successful": [{"email": "test@example.com", "message": "Success"}], "failed": []}
    ...
    manager.get_changes_log.return_value = [
        {"timestamp": "2023-01-01T10:00:00", "action": "assign_tier", "email": "test@example.com", "tier": "full", "admin": "admin@example.com"}
    ]
...
class TestTierManagementInterface:
    ...
    def test_create_admin_interface(self, interface):
        ...
        with patch('gradio.Tab') as mock_tab:
            ...
            with patch('gradio.HTML'), patch('gradio.Row'), patch('gradio.Column'), \
                 patch('gradio.Group'), patch('gradio.Textbox'), patch('gradio.Dropdown'), \
                 patch('gradio.Button'), patch('gradio.Dataframe'):
    ...
    def test_bulk_assign_success(self, interface, mock_tier_manager):
        ...
        mock_tier_manager.bulk_assign_tier.assert_called_once_with(
            ["user1@example.com", "user2@example.com"], "full", "admin@example.com"
        )
    ...
    def test_search_users_empty_query(self, interface, mock_tier_manager):
        ...
        with patch.object(interface, '_refresh_all_data') as mock_refresh:
            ...
            result = interface._search_users("")
    ...
    def test_refresh_all_data_exception(self, interface, mock_tier_manager):
        ...
        assert len(result) == 5
        assert "Refresh error" in result[0]
    ...
    def test_refresh_changes_log_empty(self, interface, mock_tier_manager):
        ...
        assert "No recent changes recorded." == result
