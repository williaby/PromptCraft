...
class TestSanitizeValidationErrors:
    ...
    def test_sanitize_no_sensitive_data(self):
        ...
        errors = [
            "Configuration field 'port' must be positive",
...
class TestGetConfigurationStatus:
    ...
    def test_get_status_value_error(self):
        ...
        with patch('src.config.health.validate_encryption_available', return_value=True):
            with patch('src.config.health._count_configured_secrets', return_value=0):
                with patch('src.config.health._determine_config_source', return_value="defaults"):
                    with patch('src.config.health.validate_configuration_on_startup', side_effect=ValueError("Format error")):
                        status = get_configuration_status(settings)
    ...
    def test_get_status_type_error(self):
        ...
        with patch('src.config.health.validate_encryption_available', return_value=True):
            with patch('src.config.health._count_configured_secrets', return_value=0):
                with patch('src.config.health._determine_config_source', return_value="defaults"):
                    with patch('src.config.health.validate_configuration_on_startup', side_effect=TypeError("Type error")):
                        status = get_configuration_status(settings)
    ...
    def test_get_status_attribute_error(self):
        ...
        with patch('src.config.health.validate_encryption_available', return_value=True):
            with patch('src.config.health._count_configured_secrets', return_value=0):
                with patch('src.config.health._determine_config_source', return_value="defaults"):
                    with patch('src.config.health.validate_configuration_on_startup', side_effect=AttributeError("Attribute error")):
                        status = get_configuration_status(settings)
    ...
    def test_get_status_unexpected_exception(self):
        ...
        with patch('src.config.health.validate_encryption_available', return_value=True):
            with patch('src.config.health._count_configured_secrets', return_value=0):
                with patch('src.config.health._determine_config_source', return_value="defaults"):
                    with patch('src.config.health.validate_configuration_on_startup', side_effect=RuntimeError("Unexpected error")):
                ...
                with patch('src.config.health._determine_config_source', return_value="defaults"):
                    with patch('src.config.health.validate_configuration_on_startup', side_effect=validation_error):
                        with patch('src.config.health._sanitize_validation_errors', return_value=["Sanitized errors"]):
                            with patch('src.config.health.get_mcp_configuration_health', return_value={"healthy": False}):
                                result = await checker.check_health()
