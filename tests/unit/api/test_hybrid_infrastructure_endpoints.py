...
class TestMCPServerEndpoints:
    ...
    async def test_get_mcp_servers_status_success(self, app, mock_request):
        ...
        app.state.mcp_manager.get_connection_status = AsyncMock(return_value={
...
class TestScriptsEndpoints:
    ...
    async def test_get_scripts_by_category_success(self, app, mock_request):
        ...
        mock_scripts = [
            MagicMock(
                script_id="deploy1", name="Deploy 1", description="First deploy",
                source_type="file", script_type="bash", is_executable=True,
            ),
        ]
