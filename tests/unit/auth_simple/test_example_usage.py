...
def setup_mocks():
    ...
    with patch("src.auth_simple.get_config_manager") as mock_get_config, \
        ...
        mock_config.config.email_whitelist = ["dev@example.com", "@example.com", "admin@example.com", "test@example.com"]
...
class TestAuthCheckEndpoint:
    ...
    def test_check_auth_status_admin_user(self, client, setup_mocks):
        ...
        # Mock both functions within the example_usage module
        with patch.object(example_usage, 'get_current_user', return_value=mock_admin_user), \
...
class TestApplicationEvents:
    ...
    def test_startup_event(self, setup_mocks):
        ...
        startup_handlers = [
            handler for handler in example_usage.app.router.on_startup
        ]
    ...
    def test_shutdown_event(self, setup_mocks):
        ...
        shutdown_handlers = [
            handler for handler in example_usage.app.router.on_shutdown
        ]
