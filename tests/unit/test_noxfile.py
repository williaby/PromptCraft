...
class TestBasicSessions:
    ...
    def test_tests_unit_without_codecov_token(self, mock_session):
        ...
        codecov_calls = [call for call in mock_session.run.call_args_list 
                        if "codecov" in str(call)]
    ...
    def test_tests_integration_with_codecov_token(self, mock_session):
        ...
        # Verify codecov upload was called
        mock_session.run.assert_any_call("codecov", "-f", "coverage-integration.xml", "-F", "integration", external=True)
...
class TestDASTScanning:
    ...
    def test_dast_scanning_session_success(self, mock_session):
        ...
        zap_calls = [call for call in mock_session.run.call_args_list 
                    if "owasp/zap2docker-stable" in str(call)]
    ...
    def test_dast_scanning_application_not_running(self, mock_session):
        ...
        def side_effect(*args, **kwargs):
            if "curl" in args and "health" in args:
                ...
                return None  # Directory creation succeeds
            else:
                return None  # Other calls succeed
        ...
        warning_calls = [call for call in mock_session.log.call_args_list 
...
class TestSessionIntegration:
    ...
    def test_full_testing_workflow(self, mock_session):
        ...
        install_calls = [call for call in mock_session.run.call_args_list 
                        if "poetry" in call[0] and "install" in call[0]]
...
class TestDASTSecurityReportGeneration:
    ...
    def test_dast_security_report_generation(self, mock_session):
        ...
        python_calls = [call for call in mock_session.run.call_args_list 
                       if call[0][0] == "python" and "-c" in call[0]]
...
class TestAdvancedMutationTestingErrorHandling:
    ...
    def test_mutation_testing_partial_report_generation(self, mock_session):
        ...
        def side_effect(*args, **kwargs):
            if "mutmut" in args and "run" in args:
                raise Exception("Mutation testing failed")
            elif "mutmut" in args and "html" in args:
                return None  # HTML report generation succeeds
            return None
...
class TestComplexScenarios:
    ...
    def test_ci_environment_behavior(self, mock_session_with_env):
        ...
        codecov_calls = [call for call in mock_session_with_env.run.call_args_list 
                        if "codecov" in str(call)]
