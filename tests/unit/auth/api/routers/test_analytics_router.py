...
def mock_suspicious_activity_detector():
    ...
    detector = AsyncMock(spec=SuspiciousActivityDetector)
    return detector
        ...
        mock_suspicious_activity_detector.analyze_behavioral_patterns.return_value = [
            {
                ...
                "affected_users": 12,
                "risk_score": 65,
                "first_observed": datetime.now(timezone.utc) - timedelta(days=7),
                "last_observed": datetime.now(timezone.utc) - timedelta(hours=2),
                "frequency": "daily"
            },
            {
                "pattern_id": "pattern_002",
                ...
                "affected_users": 3,
                "risk_score": 85,
                "first_observed": datetime.now(timezone.utc) - timedelta(days=2),
                "last_observed": datetime.now(timezone.utc) - timedelta(hours=1),
                "frequency": "sporadic"
            }
        ]
...
class TestIncidentInvestigationEndpoint:
    ...
    async def test_investigate_incident_success(self, test_client, mock_security_service, sample_investigation_request):
        ...
        investigation_response = IncidentInvestigationResponse(
            ...
            results=[
                InvestigationResult(
                    ...
                    related_events=15,
                    timeline=[{"timestamp": datetime.now(timezone.utc), "event": "login_attempt"}],
    ...
    async def test_investigate_incident_empty_result(self, test_client, mock_security_service, sample_investigation_request):
        ...
        empty_investigation = IncidentInvestigationResponse(
...
class TestAnalyticsModels:
    ...
    def test_behavior_pattern_model(self):
        ...
        pattern = BehaviorPattern(
            ...
            affected_users=5,
            risk_level="medium",
            first_observed=datetime.now(timezone.utc) - timedelta(days=1),
