...
from src.auth.models import SecurityEventSeverity, SecurityEventType, SecurityEventResponse
...
class TestUtilityMethods:
    ...
    async def test_get_detection_stats(self):
        ...
        detector._detected_patterns.extend([
            ("brute_force", datetime.now(timezone.utc), {}),
            ("brute_force", datetime.now(timezone.utc), {}),
            ("api_abuse", datetime.now(timezone.utc), {}),
        ])
        detector._suspicious_entities.add("user:bad-actor")
        detector._activity_log["user:test"] = [(datetime.now(timezone.utc), {})]
