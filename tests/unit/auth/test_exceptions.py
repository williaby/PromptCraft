...
class TestAuthExceptionHandler:
    ...
    def test_handle_authentication_error_with_user_identifier_only(self):
        ...
        with patch("src.auth.exceptions.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with("Authentication failed: Token expired - user: user456")
    ...
    def test_handle_not_found_error_with_identifier(self):
        ...
        with patch("src.auth.exceptions.logger") as mock_logger:
            exception = AuthExceptionHandler.handle_not_found_error("user", "test@example.com")
    ...
    def test_handle_rate_limit_error_with_custom_params(self):
        ...
        with patch("src.auth.exceptions.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with("Rate limit exceeded: API quota exceeded - client: client123")
    ...
    def test_handle_internal_error_basic(self):
        ...
        with patch("src.auth.exceptions.logger") as mock_logger:
            exception = AuthExceptionHandler.handle_internal_error("Database operation", test_error)

            assert isinstance(exception, HTTPException)
            assert exception.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert exception.detail == "Internal server error"
            mock_logger.error.assert_called_once_with("Database operation failed: Test error", exc_info=True)
...
class TestLoggingIntegration:
    ...
    def test_sensitive_information_not_logged(self):
        ...
        with patch("src.auth.exceptions.logger") as mock_logger:
            ...
            AuthExceptionHandler.handle_internal_error("Database connection", sensitive_error, expose_error=False)
...
class TestExceptionHeaders:
    ...
    def test_service_unavailable_has_retry_after_header(self):
        ...
        exception = AuthExceptionHandler.handle_service_unavailable("database", retry_after=300)
...
class TestEdgeCases:
    ...
    def test_none_parameters_handling(self):
        ...
        with patch("src.auth.exceptions.logger"):
            ...
            exception = AuthExceptionHandler.handle_validation_error("Test error", field_name=None, log_additional=None)
    ...
    def test_unicode_and_special_characters(self):
        ...
        with patch("src.auth.exceptions.logger"):
            ...
            exception = AuthExceptionHandler.handle_not_found_error("user", detail=unicode_message)
